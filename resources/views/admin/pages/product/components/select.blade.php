<div class="">
 <div class="select_body d-flex justify-content-between align-items-center">
   <select name="{{ $name }}{{ $attributes?'[]' : '' }}" id="select{{ $name }}" class="form-control {{ $class }}" {{ $attributes }}>
    ...
    @if($collection == $categories ||  $collection == $sub_categories)
    @else
    <button class="btn btn-info" data-toggle="modal" data-target="#{{$name}}Modal" type="button" title="{{$name}}">
...
<div class="modal fade" id="{{ $name }}Modal" tabindex="-1" aria-labelledby="{{ $name }}ModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        ...
        <forms class="component_form" data-target_select="#select{{ $name }}" action="{{ $action }}" method="POST" enctype="multipart/form-data">
